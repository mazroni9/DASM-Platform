...
import { Car, Clock, AlertCircle, Check, X, ChevronRight, ChevronLeft } from 'lucide-react';
...
export default function CurrentCar({ car }: CurrentCarProps) {
  ...
  if (!car) {
    return (
      <div className="bg-white p-6 rounded-lg shadow-md text-center">
        ...
        <h2 className="text-xl font-bold text-gray-700 mb-2">لا توجد سيارة حالية</h2>
        <p className="text-gray-500">انقر على "السيارة التالية" لبدء المزاد</p>
      </div>
    );
  }
  ...
  const getStatusText = (status: string) => {
    switch(status) {
      case 'active': return 'جارٍ المزاد';
      case 'sold': return 'تم البيع';
      case 'unsold': return 'لم يتم البيع';
    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden">
      ...
      <div className="p-4">
        ...
        <div className="bg-teal-50 p-3 rounded-lg mb-4 text-center border border-teal-200">
          ...
          <p className="text-3xl font-bold text-teal-800">{formatMoney(car.current_price)} ريال</p>
        ...
        {/* وصف السيارة */}
        <div className="mb-4">
          <h4 className="text-sm font-semibold text-gray-700 mb-1">الوصف</h4>
        ...
        {car.seller_name && (
          <div className="border-t pt-3">
            <h4 className="text-sm font-semibold text-gray-700 mb-1">البائع</h4>
            <p className="text-sm text-gray-600">{car.seller_name}</p>
          </div>
        )}
      </div>
    </div>
