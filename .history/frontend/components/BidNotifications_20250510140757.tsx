...
import { Bell, BellOff, Car, Clock, Check, X, Volume2, VolumeX } from 'lucide-react';
import { formatMoney } from '@/app/lib/format-utils';

interface Notification {
  id: string;
  type: 'outbid' | 'upcoming' | 'priceChange' | 'auctionStart' | 'auctionEnd' | 'bidAccepted';
  title: string;
  message: string;
  timestamp: number;
  isRead: boolean;
...
export default function BidNotifications({ userId, notificationCount = 3 }: BidNotificationsProps) {
    ...
    const mockNotifications: Notification[] = [
      {
        ...
        type: 'outbid',
        title: 'تم تجاوز عرضك!',
        message: 'قام مزايد آخر بتقديم عرض أعلى من عرضك على سيارة تويوتا كامري 2020 بقيمة 110,000 ريال',
      ...
      },
      {
        ...
        message: 'سيتم عرض سيارة نيسان باترول 2022 في المزاد خلال 10 دقائق، وهي تطابق بحثك السابق',
      ...
      },
      {
        ...
        type: 'bidAccepted',
        title: 'تم قبول عرضك!',
        message: 'تهانينا! تم قبول عرضك على سيارة مرسيدس E200 2019 بقيمة 150,000 ريال',
  ...
  const playNotificationSound = () => {
    if (sound && audioRef.current) {
      audioRef.current.play().catch(e => console.log('فشل تشغيل الصوت:', e));
    }
  };
  ...
  useEffect(() => {
    const mockNotificationsInterval = setInterval(() => {
      const newNotification: Notification = {
        ...
        title: Math.random() > 0.5 ? 'تم تجاوز عرضك!' : 'تغيّر سعر سيارة!',
        message: Math.random() > 0.5 
          ? 'قام مزايد آخر بتقديم عرض أعلى على سيارة هيونداي توسان 2021 بقيمة ' + formatMoney(Math.floor(Math.random() * 50000) + 80000) + ' ريال'
          : 'تغيّر سعر سيارة كيا K5 2021 إلى ' + formatMoney(Math.floor(Math.random() * 40000) + 90000) + ' ريال',
        timestamp: Date.now(),
        isRead: false,
        itemId: Math.floor(Math.random() * 10) + 120,
        itemTitle: Math.random() > 0.5 ? 'هيونداي توسان 2021' : 'كيا K5 2021'
      };
  ...
  const getNotificationIcon = (type: string) => {
    switch(type) {
      case 'outbid':
        return <div className="bg-red-100 p-2 rounded-full"><Bell className="h-5 w-5 text-red-500" /></div>;
      case 'upcoming':
        return <div className="bg-blue-100 p-2 rounded-full"><Clock className="h-5 w-5 text-blue-500" /></div>;
      case 'priceChange':
        return <div className="bg-yellow-100 p-2 rounded-full"><Bell className="h-5 w-5 text-yellow-500" /></div>;
      case 'bidAccepted':
        return <div className="bg-green-100 p-2 rounded-full"><Check className="h-5 w-5 text-green-500" /></div>;
      default:
        return <div className="bg-gray-100 p-2 rounded-full"><Car className="h-5 w-5 text-gray-500" /></div>;
    }
  };
  ...
  return (
    <div className="relative inline-block" ref={notificationsRef}>
      ...
      {/* مربع الإشعارات */}
      {showNotifications && (
        <div className="absolute left-0 mt-2 w-80 bg-white rounded-lg shadow-xl border border-gray-200 z-50" style={{ maxHeight: '500px' }}>
          {/* رأس مربع الإشعارات */}
