/**
 * 🧩 مكون سجل تاريخي للمزاد
 * 📁 المسار: components/archive/AuctionHistoryRecord.tsx
 *
 * ✅ الوظيفة:
 * - عرض سجل تاريخي لمزاد سابق مع بياناته
 * - يوفر وصلة للوصول إلى تفاصيل المزاد وفيديو التسجيل
 * - يعرض إحصائيات المزاد والمبيعات
 */
...
import { formatMoney, formatDate, formatTime } from '@/app/lib/format-utils';
...
export default function AuctionHistoryRecord({ 
  ...
  const sellRate = auction.soldCars > 0 
    ? Math.round((auction.soldCars / auction.totalCars) * 100) 
    : 0;
  
  // حساب متوسط سعر البيع
  const averageSalePrice = auction.soldCars > 0 
    ? auction.totalSales / auction.soldCars 
    : 0;
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow duration-300">
        ...
        <div className="p-4">
          <h3 className="text-lg font-semibold text-gray-800 mb-2 hover:text-teal-600 transition-colors">
            <Link href={`/auction-archive/${auction.id}`} className="hover:underline">
              {auction.title}
            </Link>
          </h3>
          
          <div className="text-sm text-gray-600 mb-3">
            <p>{auction.venue} - {auction.location}</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            ...
            <div className="bg-gray-50 p-2 rounded">
              <div className="text-xs text-gray-500 mb-1">معدل البيع</div>
              <div className="text-lg font-semibold text-teal-600">
                {sellRate}%
              </div>
            </div>
            ...
            <div className="border-t border-gray-100 pt-3 mt-3">
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div className="flex items-center">
                  ...
                  <span className="text-gray-600">{auction.soldCars} / {auction.totalCars} سيارة</span>
                </div>
                <div className="flex items-center">
                  <Users className="h-4 w-4 text-gray-400 ml-1" />
                  <span className="text-gray-600">{auction.totalParticipants} مشارك</span>
                </div>
                <div className="flex items-center">
                  <TrendingUp className="h-4 w-4 text-gray-400 ml-1" />
                  <span className="text-gray-600">{auction.totalBids} مزايدة</span>
                </div>
                <div className="flex items-center">
                  <Tag className="h-4 w-4 text-gray-400 ml-1" />
                  <span className="text-gray-600">متوسط {formatMoney(averageSalePrice)}</span>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-3">
            <div className="flex items-center">
              ...
              <span className="text-teal-600 font-semibold">{formatMoney(auction.totalSales)} ريال</span>
            </div>
            <div className="flex items-center">
              <Car className="h-4 w-4 text-blue-500 ml-1" />
              <span>{auction.soldCars} / {auction.totalCars} سيارة</span>
            </div>
            <div className="flex items-center">
