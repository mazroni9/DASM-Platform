/**
 * 🧩 مكون قائمة السيارات المباعة في المزاد
 * 📁 المسار: components/archive/AuctionCarList.tsx
 *
 * ✅ الوظيفة:
 * - عرض قائمة السيارات المباعة في المزاد
 * - إظهار تفاصيل السيارة، سعر البداية، وسعر البيع
 * - إمكانية الانتقال إلى لحظة بيع السيارة في الفيديو
 */
...
export default function AuctionCarList({ 
  ...
  const calculatePriceIncrease = (startPrice: number, finalPrice: number): number => {
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm">
        <div className="p-3 border-b flex items-center justify-between">
          <h3 className="text-lg font-semibold text-gray-800">السيارات المباعة</h3>
          <span className="text-sm text-gray-500">{cars.length} سيارة</span>
        </div>
        
        <div className="divide-y divide-gray-100">
          {sortedCars.map(car => (
            <div key={car.id} className="p-3 hover:bg-gray-50 flex justify-between items-center">
              <div className="flex items-center space-x-3 rtl:space-x-reverse">
                ...
                <div>
                  <div className="font-medium text-gray-800">{car.make} {car.model}</div>
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm">
      <div className="p-4 border-b flex flex-wrap items-center justify-between gap-y-2">
        <h3 className="text-lg font-semibold text-gray-800">السيارات المباعة في المزاد</h3>
      ...
      <div className="divide-y divide-gray-100">
        {sortedCars.length > 0 ? (
          sortedCars.map(car => (
            <div key={car.id} className="hover:bg-gray-50 transition-colors">
              ...
              <div className="p-4 flex flex-col md:flex-row md:items-center gap-4">
                ...
                <div className="flex-1">
                  <div className="flex flex-wrap justify-between items-start mb-2">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        title={expandedCarId === car.id ? "إخفاء التفاصيل" : "عرض التفاصيل"}
                      >
                        {expandedCarId === car.id ? "إخفاء التفاصيل" : "عرض التفاصيل"}
                      </button>
                      
                      {onJumpToTimestamp && (
                        <button
                          onClick={() => handleJumpToTimestamp(car.timestamp)}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-2">
                    <div>
                      <div className="text-xs text-gray-500">سعر البداية</div>
                      <div className="text-sm font-medium">{formatMoney(car.startPrice)} ريال</div>
                    </div>
                    
                    <div>
                      <div className="text-xs text-gray-500">سعر البيع</div>
                      <div className="text-sm font-semibold text-teal-600">{formatMoney(car.finalPrice)} ريال</div>
                    </div>
                    
                    <div>
                      <div className="text-xs text-gray-500">الزيادة</div>
                      <div className="text-sm font-medium flex items-center text-green-600">
                        <TrendingUp className="h-3 w-3 ml-1" />
                        {calculatePriceIncrease(car.startPrice, car.finalPrice)}%
                      </div>
                    </div>
                    
                    <div>
                      <div className="text-xs text-gray-500">المزايدات</div>
                      <div className="text-sm font-medium">{car.bidCount} مزايدة</div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center text-xs text-gray-500">
                    ...
                    <span>
                      وقت البيع: {formatTimeDisplay(car.timestamp)} | 
                      مدة المزايدة: {formatTimeDisplay(car.duration)} | 
              ...
              {expandedCarId === car.id && (
                <div className="p-4 bg-gray-50 border-t border-gray-100">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    {car.fuelType && (
                      <div>
                        <div className="text-xs text-gray-500 mb-1">نوع الوقود</div>
                        <div className="text-sm">{car.fuelType}</div>
                      </div>
                    )}
                    
                    {car.transmission && (
                      <div>
                        <div className="text-xs text-gray-500 mb-1">ناقل الحركة</div>
                        <div className="text-sm">{car.transmission}</div>
                      </div>
                    )}
                    
                    <div>
                      <div className="text-xs text-gray-500 mb-1">معدل المزايدة</div>
                      <div className="text-sm">
                        {(car.bidCount / (car.duration / 60)).toFixed(1)} مزايدة/دقيقة
                      </div>
                    </div>
                    
                    <div>
                      <div className="text-xs text-gray-500 mb-1">متوسط الزيادة للمزايدة</div>
                      <div className="text-sm">
                        {formatMoney((car.finalPrice - car.startPrice) / (car.bidCount || 1))} ريال
                      </div>
                    </div>
                  </div>
                </div>
