...
export default function AdminProcessAuctionPage() {
    ...
    const [activeTab, setActiveTab] = useState<"info" | "pricing" | "decision">("info");
    ...
    const fetchCarData = async () => {
        try {
            const response = await api.get(`/api/admin/cars/${carId}/process-auction`);
    ...
    const handleApprove = async () => {
        ...
        try {
            setProcessing(true);
            const response = await api.post(`/api/admin/cars/${carId}/handle-auction`, {
                action: "approve",
        ...
        return (
            <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-950 flex items-center justify-center">
                <div className="text-center">
                    ...
                    <h3 className="text-lg font-medium text-gray-400">لم يتم العثور على البيانات</h3>
                    <p className="text-gray-500 mt-1">لا يوجد طلب مزاد معلق لهذه السيارة</p>
                </div>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-950 text-white p-2 rtl">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8">
                ...
                <div className="flex items-center space-x-3 space-x-reverse mt-4 lg:mt-0">
                    ...
                    >
                        {editMode ? <X className="w-4 h-4 ml-2" /> : <Edit3 className="w-4 h-4 ml-2" />}
                                ...
                                <div className="space-y-6">
                                    ...
                                    {car.images && car.images.length > 0 && (
                                        <div className="bg-gray-700/30 rounded-xl p-6">
                                            <div className="flex items-center space-x-3 space-x-reverse mb-4">
                                                ...
                                                <h3 className="text-lg font-semibold text-white">صور السيارة</h3>
                                            </div>
                                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                                {car.images.slice(0, 4).map((image, index) => (
                                                    <div key={index} className="relative h-32 rounded-xl overflow-hidden border border-gray-600">
                                                        <Image
                                                            src={image}
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="space-y-4">
                                            <div>
                                                <label className="block text-sm font-medium text-gray-300 mb-2">الماركة</label>
                                                {editMode ? (
                                                    <input
                                                        type="text"
                                                        value={formData.make}
                                                    ...
                                                    <p className="text-white bg-gray-700/30 rounded-xl py-3 px-4">{car.make}</p>
                                                )}
                                            </div>
                                            <div>
                                                <label className="block text-sm font-medium text-gray-300 mb-2">الموديل</label>
                                                {editMode ? (
                                                    <input
                                                        type="text"
                                                    ...
                                                    <p className="text-white bg-gray-700/30 rounded-xl py-3 px-4">{car.model}</p>
                                                )}
                                            </div>
                                            <div>
                                                <label className="block text-sm font-medium text-gray-300 mb-2">سنة الصنع</label>
                                                {editMode ? (
                                                    <input
                                                        type="number"
                                                        value={formData.year}
                                                    ...
                                                    <p className="text-white bg-gray-700/30 rounded-xl py-3 px-4">{car.year}</p>
                                                )}
                                            </div>
                                        </div>
                                        <div className="space-y-4">
                                            <div>
                                                ...
                                                {editMode ? (
                                                    ...
                                                    <p className="text-white bg-gray-700/30 rounded-xl py-3 px-4">{getConditionText(car.condition)}</p>
                                                )}
                                            </div>
                                            <div>
                                                <label className="block text-sm font-medium text-gray-300 mb-2">ناقل الحركة</label>
                                                {editMode ? (
                                                    <select
                                                    ...
                                                    <p className="text-white bg-gray-700/30 rounded-xl py-3 px-4">{getTransmissionText(car.transmission)}</p>
                                                )}
                                            </div>
                                            <div>
                                                <label className="block text-sm font-medium text-gray-300 mb-2">عداد الكيلومترات</label>
                                                {editMode ? (
                                                    <input
                                                        type="number"
                                                        value={formData.odometer}
                                                        onChange={(e) => setFormData(prev => ({ ...prev, odometer: parseInt(e.target.value) }))}
                                                        className="w-full bg-gray-700/50 border border-gray-600 rounded-xl py-3 px-4 text-white focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                                                    />
                                                ) : (
                                                    <p className="text-white bg-gray-700/30 rounded-xl py-3 px-4">{car.odometer?.toLocaleString()} كم</p>
                                                )}
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="space-y-6">
                                    <div className="bg-gray-700/30 rounded-xl p-6">
                                        <div className="flex items-center space-x-3 space-x-reverse mb-6">
                                            ...
                                            <h3 className="text-lg font-semibold text-white">عروض المالك</h3>
                                        </div>
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                            <div className="bg-gray-600/30 rounded-xl p-4 text-center">
                                                <div className="text-amber-400 text-sm mb-2">الحد الأدنى</div>
                                                <div className="text-2xl font-bold text-white">{auction.minimum_bid?.toLocaleString()}</div>
                                                <div className="text-gray-400 text-sm">ريال</div>
                                            </div>
                                            <div className="bg-gray-600/30 rounded-xl p-4 text-center">
                                                <div className="text-amber-400 text-sm mb-2">الحد الأقصى</div>
                                                <div className="text-2xl font-bold text-white">{auction.maximum_bid?.toLocaleString()}</div>
                                                <div className="text-gray-400 text-sm">ريال</div>
                                            </div>
                                            <div className="bg-gray-600/30 rounded-xl p-4 text-center">
                                                <div className="text-amber-400 text-sm mb-2">السعر الاحتياطي</div>
                                                <div className="text-2xl font-bold text-white">{auction.reserve_price?.toLocaleString()}</div>
                                                <div className="text-gray-400 text-sm">ريال</div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-gray-700/30 rounded-xl p-6">
                                        <div className="flex items-center space-x-3 space-x-reverse mb-6">
                                            ...
                                            <h3 className="text-lg font-semibold text-white">التسعير المقترح</h3>
                                        </div>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <div>
                                                <label className="block text-sm font-medium text-gray-300 mb-2">السعر الافتتاحي للمزاد *</label>
                                                <input
                                                    type="number"
                                                    value={formData.opening_price}
                                                    onChange={(e) => setFormData(prev => ({ ...prev, opening_price: parseFloat(e.target.value) }))}
                                                    className="w-full bg-gray-700/50 border border-gray-600 rounded-xl py-3 px-4 text-white focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                                                    placeholder="أدخل السعر الافتتاحي"
                                                />
                                            </div>
                                            <div>
                                                <label className="block text-sm font-medium text-gray-300 mb-2">سعر التقييم *</label>
                                                <input
                                                    type="number"
                                                    value={formData.evaluation_price}
                                                    onChange={(e) => setFormData(prev => ({ ...prev, evaluation_price: parseFloat(e.target.value) }))}
                                                    className="w-full bg-gray-700/50 border border-gray-600 rounded-xl py-3 px-4 text-white focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                                ...
                                <div className="space-y-6">
                                    <div className="bg-gray-700/30 rounded-xl p-6">
                                        <div className="flex items-center space-x-3 space-x-reverse mb-6">
                                            ...
                                            <h3 className="text-lg font-semibold text-white">قرار المعالجة النهائي</h3>
                                        </div>
                                        
                                        <div className="mb-6">
                                            <label className="block text-sm font-medium text-gray-300 mb-2">سبب الرفض (في حالة الرفض)</label>
                                            <textarea
