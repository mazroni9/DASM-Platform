...
export default function AdminEditBlogPostPage() {
  ...
  const [form, setForm] = useState({
    ...
    // ✅ UI field (mapped to is_published in API)
  ...
  const fetchPost = async () => {
    try {
      ...
      toast.error(err?.response?.data?.message || "فشل في تحميل المقال");
  ...
  const slugHelp = useMemo(() => {
    ...
    if (!s) return { type: "warn" as const, text: "الـSlug ضروري للرابط" };
  ...
  const validate = () => {
    ...
    if (!slug) return { ok: false, msg: "الـSlug مطلوب" };
    if (!form.category_id) return { ok: false, msg: "التصنيف مطلوب" };

    if (title.length > 255) return { ok: false, msg: "العنوان طويل جدًا" };
    if (slug.length > 255) return { ok: false, msg: "الـSlug طويل جدًا" };
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-foreground/70 mt-1">تعديل و معاينة قبل الحفظ</p>
            </div>
          </div>
        </div>
            ...
            <div className="bg-card border border-border rounded-2xl shadow-2xl overflow-hidden">
              <div className="border-b border-border p-5 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-bold">{tab === "edit" ? "تحرير المحتوى" : "معاينة المقال"}</h2>
                </div>
          ...
          <div className="xl:col-span-4">
            <div className="bg-card border border-border rounded-2xl shadow-2xl overflow-hidden">
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  <label className="block text-sm font-bold mb-2">تاريخ النشر (اختياري)</label>
                  <input
                    type="datetime-local"
