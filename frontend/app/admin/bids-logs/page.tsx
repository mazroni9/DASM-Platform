...
export default function BidsLogsPage() {
    ...
    const [processingBidsLogsId, setProcessingBidsLogsId] = useState<number | null>(
        null
    );
    ...
    const fetchBidsLogs = async () => {
        try {
            const response = await api.get(`/api/admin/bids/events?page=${currentPage}`);

            if (response.data && response.data.status === "success") {
                console.log("Fetched bids logs:", response.data);
