    ...
    async function fetchAuctions() {
      ...
      try {
        ...
        if (response.data.data || response.data.data) {
          ...
          const live_instant = carsData.filter(
            (car: any) => car.auction_type === "live_instant"
          );
      ...
      setInterval(async () => {
        for (let auctionId of auctionIds) {
          ...
          try {
            ...
            console.error(
              `Error in auction ${auctionId}, user ${userId}:`,
              error.response?.data || error.message
            );
          }

          // تحديث مؤشر المستخدم للمزاد
          currentIndexMap[auctionId] =
            (currentIndexMap[auctionId] + 1) % userIds.length;
        }
        fetchAuctions();
      }, 5000); // كل 10 ثواني
    }
                  ...
                  <tr key={idx} className="border-t hover:bg-gray-50">
                    {car.auction_type != "live" &&
                      car["car"].auction_status == "in_auction" && (
                        <>
                          <td className="p-2 text-sm">{car["car"].province}</td>
                          <td className="p-2 text-sm">{car["car"].city}</td>
                          ...
                          <td className="p-2 text-sm">{car["car"].odometer}</td>
                          ...
                          <td className="p-2 text-sm text-blue-600 underline">
                            <Link href={`/carDetails/${car.car_id}`}>عرض</Link>
                          </td>
                        </>
                      )}
                  </tr>
