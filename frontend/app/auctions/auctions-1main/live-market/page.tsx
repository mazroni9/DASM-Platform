...
export default function LiveMarketPage() {
    ...
    // Fetch broadcast and current car data
    useEffect(() => {
        async function fetchBroadcastData() {
            ...
            setLoading(true);
            try {
                // Fetch current broadcast info and current car
                ...
                if (broadcastResponse.data.status === "success") {
                    ...
                    if (broadcast && broadcast.current_car) {
                        setCurrentCar(broadcast.current_car);

                        // Check if current user is owner of the current car
                        const car = broadcast.current_car;
                        const current_user_id = user.id;
                        const car_user_id = car.owner?.id;
                        const dealer_user_id = car.dealer?.id;
                ...
                // Still fetch other live auctions for the sidebar
                const auctionsResponse = await api.get(
                    "/api/approved-auctions"
                );
                if (auctionsResponse.data.data || auctionsResponse.data.data) {
                    const carsData =
                        auctionsResponse.data.data.data ||
                ...
                console.error("فشل تحميل بيانات البث المباشر", error);
                setBroadcastData(null);
                setCurrentCar(null);
                setMarketCars([]);
                setMarketCarsCompleted([]);
        ...
        const interval = setInterval(fetchBroadcastData, 10000);
                    ...
                    <div className="flex items-center justify-center min-h-[400px]">
                        <div className="text-center">
                            ...
                            <p className="text-gray-500">
                                سيتم إشعارك عند بدء البث المباشر التالي
                            </p>
                        </div>
                    </div>
                ) : (
                            ...
                            <div className="bg-white p-4 rounded-xl shadow-md">
                                ...
                                {currentCar ? (
                                            ...
                                            <div className="mt-3 border rounded-lg bg-gray-50 p-3">
                                                ...
                                                <div className="text-center mb-3">
                                                    ...
                                                    <div className="text-2xl font-bold text-teal-600 my-2 py-2 rounded-lg border-2 border-teal-200 bg-white">
                                                        {formatMoney(
                                                            currentCar.current_bid ||
                                                                currentCar.starting_price ||
                                                                "0"
                                                        )}
                                                    </div>
                                                </div>
                                    ...
                                    <div className="h-full flex items-center justify-center">
                                        <p className="text-gray-500">
                                            لا توجد سيارة معروضة حاليًا في
                                            الحراج المباشر
                                        </p>
                                    </div>
                                )}
