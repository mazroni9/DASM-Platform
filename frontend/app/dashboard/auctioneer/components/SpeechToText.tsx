  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className={`h-3 w-3 rounded-full ${isListening ? 'bg-red-500 animate-pulse' : 'bg-gray-300'}`}></div>
      </div>
      ...
      <div className="space-y-4">
        ...
        {transcript && (
          <div className="bg-gray-50 rounded-lg p-4">
            <p className="text-sm text-gray-600 mb-2">النص المُسجّل:</p>
            <p className="text-gray-900 font-medium">{transcript}</p>
          </div>
        )}

        <div className="text-xs text-gray-500">
          {isListening ? 'يتم الاستماع...' : 'اضغط على "بدء التسجيل" للبدء'}
        </div>
      </div>
    </div>
  );
