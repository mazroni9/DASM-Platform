...
export default function CouncilArticleNewPage() {
  ...
  useEffect(() => {
    if (can("council.article.create")) fetchCategories();
  }, [can]);
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      setSaving(true);
      await api.post("/api/council-studio/articles", {
  ...
  return (
    <div className="space-y-6 rtl">
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
        <div>
          <h1 className="text-2xl font-bold text-primary">مقال جديد</h1>
          <p className="text-foreground/60 text-sm mt-1">مقال جديد لمجلس السوق</p>
        </div>
      ...
      {activeTab === "preview" ? (
        ...
        <form onSubmit={submit} className="bg-card border border-border rounded-2xl p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              ...
              {!loadingCats && !categories.length ? (
                <p className="mt-1 text-xs text-amber-600">أضف تصنيفاً من لوحة الإدارة أولاً</p>
              ) : null}
            </div>
            <div>
              ...
              >
                ...
                <option value="pending_review">إرسال للمراجعة</option>
          ...
          <div className="rounded-2xl border border-border bg-background/40 p-4">
            ...
            <p className="text-xs text-foreground/60 mb-3">اختر السياقات المناسبة للتوصية</p>
