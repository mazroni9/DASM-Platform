...
export function useOBS() {
  ...
  useEffect(() => {
    ...
    const obsInstance = obsService.getOBSService().getOBSWebSocket();
    ...
    return () => {
      obsInstance.removeListener("ConnectionOpened", handleConnectionChange);
      obsInstance.removeListener("ConnectionClosed", handleConnectionChange);
  ...
  const switchScene = async (sceneName) => {
    ...
    try {
      const obsService = getBackendOBSService();
      const result = await obsService.getOBSService().switchScene(sceneName);
  ...
  const updateCarInfo = async (carInfo, textSourceName = "معلومات_السيارة") => {
    if (!isConnected) {
      toast.error("يجب الاتصال بـ OBS Studio أولاً");
      return false;
    }
