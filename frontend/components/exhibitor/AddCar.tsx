...
export default function AddCarForm() {
  ...
  const labelFor = (key: keyof FormData): string => {
    const map: Record<keyof FormData, string> = {
      make: 'الشركة المصنعة', model: 'الموديل', year: 'سنة الصنع', vin: 'رقم الهيكل (VIN)',
      odometer: 'عدد الكيلومترات', condition: 'حالة السيارة', evaluation_price: 'السعر المقيّم',
      color: 'اللون', engine: 'سعة/نوع المحرك', transmission: 'ناقل الحركة', market_category: 'فئة السوق',
      description: 'الوصف', min_price: 'الحد الأدنى', max_price: 'الحد الأعلى',
      province: 'المحافظة', city: 'المدينة', plate: 'رقم اللوحة'
    }
    return map[key]
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!token) { setIsSubmitting(false); setErrorMsg('غير مصرح: مفقود رمز الدخول. الرجاء تسجيل الدخول أولاً.'); return }
