...
export default function LiveAuctionPulse({
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      ...
      const viewerChange =
        Math.floor(Math.random() * 4) * (Math.random() > 0.6 ? 1 : -1);
      ...
      setInterestLevel((prev) =>
        Math.min(100, Math.max(0, prev + interestChange))
      );
  ...
  const getPriceRateIndicator = (rate: number): React.ReactElement => {
    if (rate > 3) {
      return (
        ...
        <div className="grid grid-cols-4 gap-2">
          ...
          <div className="bg-primary/10 p-2 rounded-lg">
            ...
            <div className="flex items-end">
              <span className="text-xl font-bold text-primary">{viewers}</span>
              <span className="text-xs text-primary/70 mb-1 mr-1">نشط</span>
            </div>
          </div>
          ...
          <div className="bg-border p-2 rounded-lg">
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <span
                  className={`text-sm font-medium ${getInterestColor(
                    interestLevel
                  )}`}
                >
