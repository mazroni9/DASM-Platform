...
import { Building2, Calendar, GitFork, Pencil, Trash2, Users } from "lucide-react";
import type { OrganizationStatusValue, OrganizationTypeValue } from "@/lib/organization";
...
> = {
  active: {
    label: "نشط",
    className: "bg-green-500/10 text-green-700 dark:bg-green-500/20 dark:text-green-300 border-green-500/30",
  },
  inactive: {
    label: "غير نشط",
    className: "bg-neutral-500/10 text-neutral-700 dark:bg-neutral-500/20 dark:text-neutral-200 border-neutral-500/30",
  },
  pending: {
    label: "معلق",
    className: "bg-amber-500/10 text-amber-700 dark:bg-amber-500/20 dark:text-amber-200 border-amber-500/30",
  },
};
    ...
    <Card className="flex flex-col">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="icon" onClick={() => onEdit?.(organization)}>
      ...
      <CardContent className="flex-grow">
        <p className="mb-4 text-sm text-muted-foreground">المالك: {organization.ownerName}</p>
