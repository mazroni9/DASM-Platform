...
export default function PriceInfoDashboard({
  ...
  const [priceHistory, setPriceHistory] = useState<PricePoint[]>(historyPoints);
  ...
  const prevPrice = previousPrice !== undefined ? previousPrice : currentPrice;
  ...
  const getPriceDirectionIcon = () => {
    switch (priceDirection) {
      case 'up': return <ArrowUp className="h-6 w-6 text-green-600" />;
      case 'down': return <ArrowDown className="h-6 w-6 text-red-600" />;
      default: return <Minus className="h-6 w-6 text-gray-600" />;
    }
  };

  return (
    <div className={`bg-white rounded-lg shadow-md overflow-hidden ${className}`}>
      <div className={`p-3 ${auctionType === 'live_instant' ? 'bg-blue-600' : 'bg-purple-600'} text-white`}>
      ...
      <div className="p-4">
        ...
        <div className="mb-4 text-center">
          ...
          <div className={`text-4xl font-bold ${getPriceDirectionColor()} transition-colors duration-500`}>
          ...
          <div className="flex justify-center items-center mt-2">
            ...
            <span className={`mx-1 ${getPriceDirectionColor()}`}>
              {priceChange.amount > 0 ? '+' : ''}{formatMoney(priceChange.amount)} ر.س
            </span>
          ...
          {priceHistory.length > 0 ? (
            <>
              {priceHistory.map((point, index) => {
                ...
                if (index > 0) {
                  if (point.price > priceHistory[index - 1].price) {
                    barColor = 'bg-green-500';
                  } else if (point.price < priceHistory[index - 1].price) {
                    barColor = 'bg-red-500';
                  }
                }

                return (
                  <div key={index} className="flex flex-col items-center mx-[1px]">
                    <div
                      className={`w-1.5 ${barColor} rounded-t transition-all duration-500`}
                      style={{ height: `${height}%` }}
                      title={`${point.time}: ${formatMoney(point.price)} ر.س`}
                    ></div>
