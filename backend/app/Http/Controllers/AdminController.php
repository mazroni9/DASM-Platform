...
class AdminController extends Controller
{
    ...
    /**
     * Get all caravans (admin)
     * 
     * @param Request $request
     * @return \Illuminate\Http\JsonResponse
     */
    public function getAllCaravans(Request $request)
    {
        $query = \App\Models\Caravan::with(['dealer.user']);
        ...
        $caravans = $query->orderBy($request->get('sort_by', 'created_at'), $request->get('order', 'desc'))
    ...
    /**
     * Update caravan status (admin)
     * 
     * @param int $id
     * @param Request $request
     * @return \Illuminate\Http\JsonResponse
     */
    public function updateCaravanStatus($id, Request $request)
    {
        $validator = Validator::make($request->all(), [
            'auction_status' => 'required|in:pending,live,sold,expired',
        ]);
        ...
        $caravan = \App\Models\Caravan::findOrFail($id);
        $caravan->auction_status = $request->auction_status;
        $caravan->save();
        
        // If status is changed to "sold", check if there's an active auction and update it
        if ($request->auction_status === 'sold') {
            $activeAuction = $caravan->auctions()
                ->whereIn('status', [AuctionStatus::SCHEDULED, AuctionStatus::ACTIVE])
                ->first();
                
            if ($activeAuction) {
                $activeAuction->status = AuctionStatus::ENDED;
                $activeAuction->save();
            }
        }
        ...
        return response()->json([
            'status' => 'success',
            'message' => 'Caravan status updated successfully',
            'data' => $caravan
        ]);
    }
    
    /**
     * Update caravan details (admin)
     * 
     * @param int $id
     * @param Request $request
     * @return \Illuminate\Http\JsonResponse
     */
    public function updateCaravan($id, Request $request)
    {
        $caravan = \App\Models\Caravan::findOrFail($id);
        ...
        $validator = Validator::make($request->all(), [
            ...
            'auction_status' => 'sometimes|in:pending,live,sold,expired',
        ]);
    ...
    /**
     * Delete a caravan (admin)
     * 
     * @param int $id
     * @return \Illuminate\Http\JsonResponse
     */
    public function deleteCaravan($id)
    {
        $caravan = \App\Models\Caravan::findOrFail($id);
        
        // Check if caravan can be deleted (not in active auction)
        if (in_array($caravan->auction_status, ['live', 'sold'])) {
            return response()->json([
                'status' => 'error',
                'message' => 'Cannot delete a caravan that is in active auction or sold'
            ], 400);
        }
        
        // Check for related auctions
        if ($caravan->auctions()->exists()) {
            // If has auctions, check if they can be deleted
            $activeAuctions = $caravan->auctions()
                ->whereIn('status', [AuctionStatus::SCHEDULED, AuctionStatus::ACTIVE])
                ->count();
            ...
            if ($activeAuctions > 0) {
                return response()->json([
                    'status' => 'error',
                    'message' => 'Cannot delete caravan with active auctions'
                ], 400);
            }
        }
