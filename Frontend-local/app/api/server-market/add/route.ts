/**
 * 📝 الملف: مسار API لإضافة السيرفرات المستعملة
 * 📁 المسار: Frontend-local/app/api/server-market/add/route.ts
 * 
 * ✅ الوظيفة:
 * - استقبال بيانات السيرفرات المستعملة من نموذج الإدخال
 * - معالجة الصور المرفقة وملف PDF
 * - حفظ البيانات في قاعدة البيانات في جدول products
 * 
 * ✅ طريقة الربط:
 * - يستقبل طلبات POST من صفحة forms/server-market-entry
 * - يحتفظ بالصور في مجلد /auctionsPIC/quality-usedserversPIC
 * - يحتفظ بملفات PDF في مجلد /auctionsPIC/quality-usedserversPIC/pdfReports
 */
...
const IMAGES_DIRECTORY = path.join(process.cwd(), 'public', 'auctionsPIC', 'quality-usedserversPIC');
// تحديد المسار الذي سيتم حفظ ملفات PDF فيه
const PDF_DIRECTORY = path.join(process.cwd(), 'public', 'auctionsPIC', 'quality-usedserversPIC', 'pdfReports');
...
export async function POST(request: NextRequest) {
  try {
    ...
    const category = formData.get('category') as string || 'السيرفرات';

    // التحقق من البيانات الإلزامية
    if (!name || !description || !specs || !price || !condition) {
      return NextResponse.json(
    ...
    for (const imageFile of imageFiles) {
      if (imageFile.size > 0) {
        ...
        const fileName = `server_${uuidv4()}.${fileExtension}`;
        ...
        imageUrls.push(`/auctionsPIC/quality-usedserversPIC/${fileName}`);
      }
    }
    ...
    if (pdfFile && pdfFile.size > 0) {
      ...
      const fileName = `server_report_${uuidv4()}.${fileExtension}`;
      ...
      pdfUrl = `/auctionsPIC/quality-usedserversPIC/pdfReports/${fileName}`;
    }
    ...
    try {
      ...
      const result = await db.query(
        `INSERT INTO products (category, name, description, specs, condition, price, images, pdf_report, created_at, updated_at)
      ...
      return NextResponse.json({
        message: 'تمت إضافة السيرفر بنجاح',
