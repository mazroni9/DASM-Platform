/**
 * 📝 الملف: مسار API لإضافة الآلات المكتبية المستعملة
 * 📁 المسار: Frontend-local/app/api/office-market/add/route.ts
 * 
 * ✅ الوظيفة:
 * - استقبال بيانات الآلات المكتبية المستعملة من نموذج الإدخال
 * - معالجة الصور المرفقة وملف PDF
 * - حفظ البيانات في قاعدة البيانات في جدول products
 * 
 * ✅ طريقة الربط:
 * - يستقبل طلبات POST من صفحة forms/office-market-entry
 * - يحتفظ بالصور في مجلد /auctionsPIC/quality-officePIC
 * - يحتفظ بملفات PDF في مجلد /auctionsPIC/quality-officePIC/pdfReports
 */
...
const IMAGES_DIRECTORY = path.join(process.cwd(), 'public', 'auctionsPIC', 'quality-officePIC');
// تحديد المسار الذي سيتم حفظ ملفات PDF فيه
const PDF_DIRECTORY = path.join(process.cwd(), 'public', 'auctionsPIC', 'quality-officePIC', 'pdfReports');
...
export async function POST(request: NextRequest) {
  try {
    ...
    const category = formData.get('category') as string || 'الآلات المكتبية';

    // التحقق من البيانات الإلزامية
    if (!name || !description || !specs || !price || !condition || !manufacturer || !model_year || !type) {
      return NextResponse.json(
    ...
    for (const imageFile of imageFiles) {
      if (imageFile.size > 0) {
        ...
        const fileName = `office_${uuidv4()}.${fileExtension}`;
        ...
        imageUrls.push(`/auctionsPIC/quality-officePIC/${fileName}`);
      }
    }
    ...
    if (pdfFile && pdfFile.size > 0) {
      ...
      const fileName = `office_report_${uuidv4()}.${fileExtension}`;
      ...
      pdfUrl = `/auctionsPIC/quality-officePIC/pdfReports/${fileName}`;
    }
    ...
    try {
      ...
      const result = await db.query(
        `INSERT INTO products (category, name, description, specs, condition, price, manufacturer, model_year, type, images, pdf_report, created_at, updated_at)
      ...
      return NextResponse.json({
        message: 'تمت إضافة الآلة المكتبية بنجاح',
